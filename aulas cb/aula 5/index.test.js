...
test('deve retornar 200 e um JSON no GET /produtos', async function(){
...
test('deve retornar 201 e um JSON no POST /produtos', async function(){
    const response = await request.post('/produtos').body ({nome: "banana", preco: 15.00});
...
test('deve retornar 200 e um JSON no PUT /produtos/1', async function(){
    const response = await request.put('/produtos/1').body ({nome: 'banana nanica', preco: 15.00});
    expect(response.status).toBe(200);
    expect(response.headers['Content-Type']).toMatch(/json/)
})
